...
(function (context, factory) {
	...
	function bindAxis(up, down, left, right, callback) {
		...
		// Alternative right (using 'w' key)
    var clearRightAlternative = bindKey('w', function () {
        if (axis[1] === 0) {
            axis[1] = 1;
        }
    }, function () {
	...
	//  Alternative right (using 's' key)
	var clearLeft = bindKey('s', function () {
		if(axis[1] === 0) {
			axis[1] = -1;
		}
	}, function() {
	...
	//  Alternative right (using 'a' key)
	var clearUp = bindKey('a', function () {
		if(axis[0] === 0) {
			axis[0] = -1;
		}
	}, function() {
	...
	//  Alternative right (using 'd' key)
	var clearDown = bindKey('d', function () {
		if(axis[0] === 0) {
			axis[0] = 1;
		}
	}, function() {
	...
	function unbindKey(keys) {
		...
		keys = keys.concat(['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'w', 'a', 's', 'd']);
